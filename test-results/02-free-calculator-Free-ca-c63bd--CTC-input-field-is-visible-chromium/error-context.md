...
- Name: 02-free-calculator.spec.ts >> Free calculator — page structure >> CTC input field is visible
- Location: tests/e2e/02-free-calculator.spec.ts:26:7
...
Locator: locator('input[name*="ctc" i], input[name*="CTC"], input[placeholder*="CTC"], input[placeholder*="salary"]').or(getByRole('spinbutton').first()).first()
Expected: visible
Timeout: 8000ms
...
Call log:
  ...
  - waiting for locator('input[name*="ctc" i], input[name*="CTC"], input[placeholder*="CTC"], input[placeholder*="salary"]').or(getByRole('spinbutton').first()).first()
